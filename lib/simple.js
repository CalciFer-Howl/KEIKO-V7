...
`.trim()
 ...
 pass: {
      /**
       * Forward message or content to a specific group or chat.
       * @param {String} text
       * @param {String} quoted
       * @param {Object} options
       */
      value(text, quoted, options = {}) {
        if (typeof text !== 'string' || !text.includes('/')) {
          throw new Error('Invalid format. Use "groupjid/content" format.');
        }

        const [groupJid, content] = text.split('/');

        if (!groupJid || !content) {
          throw new Error('Both groupjid and content are required.');
        }

        return conn.sendMessage(groupJid, { text: content, ...options }, { quoted });
      },
      enumerable: true,
    },
    sendButton:  {
...
export function serialize() {
   ...
   */
  value(text, chatId, options) {
    const targetChat = chatId || this.chat;
    if (!this.conn || !targetChat) throw new Error('Invalid connection or chatId.');
    return this.conn.sendMessage(targetChat, { text, ...options }, { quoted: this });
  },
},
pass: {
  /**
   * Forward a message to a specific group or chat.
   * @param {String} groupJid - The target group JID.
   * @param {String} content - The message content.
   * @param {Object} options - Additional message options.
   */
  value(groupJid, content, options = {}) {
    if (!groupJid || !content) {
      throw new Error('Both groupJid and content are required.');
    }
    if (!/^[0-9]+@g.us$/.test(groupJid)) {
      throw new Error('Invalid groupJid format. It must end with "@g.us".');
    }
    return this.conn?.sendMessage(groupJid, { text: content, ...options });
  },
  enumerable: true,
},
forward: {
  /**
   * Forward a message to a specific chat.
   * @param {String} jid - The target chat ID.
   * @param {Boolean} [force=false] - Whether to force forward.
   * @param {Object} options - Additional message options.
   */
  value(jid, force = false, options = {}) {
    if (!this.conn || !jid) throw new Error('Invalid connection or chatId.');
